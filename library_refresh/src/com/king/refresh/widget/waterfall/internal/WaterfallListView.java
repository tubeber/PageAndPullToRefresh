 ...
 */
public class WaterfallListView extends WaterfallAbsListView {
    ...
    private ArrayList<FixedViewInfo> mHeaderViewInfos = new ArrayList<WaterfallListView.FixedViewInfo>();
    private ArrayList<FixedViewInfo> mFooterViewInfos = new ArrayList<WaterfallListView.FixedViewInfo>();
    ...
    public boolean removeHeaderView(View v) {
        if (mHeaderViewInfos.size() > 0) {
            ...
            if (((WaterfallHeaderViewListAdapter) mAdapter).removeHeader(v)) {
    ...
    public boolean removeFooterView(View v) {
        if (mFooterViewInfos.size() > 0) {
            boolean result = false;
            if (((WaterfallHeaderViewListAdapter) mAdapter).removeFooter(v)) {
                mDataSetObserver.onChanged();
                result = true;
            }
    ...
    public void setAdapter(ListAdapter adapter) {
        ...
        if (mHeaderViewInfos.size() > 0|| mFooterViewInfos.size() > 0) {
            mAdapter = new WaterfallHeaderViewListAdapter(mHeaderViewInfos, mFooterViewInfos, adapter);
        } else {
            mAdapter = adapter;
        }
    ...
    final int measureHeightOfChildren(int widthMeasureSpec, int startPosition, int endPosition,
        ...
        final WaterfallAbsListView.RecycleBin recycleBin = mRecycler;
    ...
    private void setupChild(View child, int position, int y, boolean flowDown, int childrenLeft,
        ...
        WaterfallAbsListView.LayoutParams p = (WaterfallAbsListView.LayoutParams) child.getLayoutParams();
        if (p == null) {
            p = new WaterfallAbsListView.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,
                    ViewGroup.LayoutParams.WRAP_CONTENT, 0);
        }
        p.viewType = mAdapter.getItemViewType(position);
        ...
        if ((recycled && !p.forceAdd) || (p.recycledHeaderFooter &&
                p.viewType == WaterfallAdapterView.ITEM_VIEW_TYPE_HEADER_OR_FOOTER)) {
            attachViewToParent(child, flowDown ? -1 : 0, p);
        } else {
            p.forceAdd = false;
            if (p.viewType == WaterfallAdapterView.ITEM_VIEW_TYPE_HEADER_OR_FOOTER) {
    ...
    private void scrollListItemsBy(int amount) {
        ...
        if (amount < 0) {
            ...
            while (first.getBottom() < listTop) {
                WaterfallAbsListView.LayoutParams layoutParams = (LayoutParams) first.getLayoutParams();
            ...
            while (last.getTop() > listBottom) {
                WaterfallAbsListView.LayoutParams layoutParams = (LayoutParams) last.getLayoutParams();
                if (recycleBin.shouldRecycleViewType(layoutParams.viewType)) {
